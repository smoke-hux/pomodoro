...
If the file cannot be read at launch — truncated by a full disk, or edited by hand into something that will not parse — Pomodoro starts fresh and says so in the window. It does not save over the old file: it is kept beside `pomodoro.json` as `pomodoro.unreadable-<timestamp>.json`, owner-only like the store itself, so it can be repaired or mined by hand.
